  ...
  const clickHandler = async (article) => {
    try {
      ...
      if (existingArticle) {
        ...
        await axios.delete(
          `https://news-together-app.onrender.com/api/v1/deleteBookmarkedNews/${existingArticle._id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {news.map((article, index) => (
          ...
          >
            <div
              className={`w-[40px] h-[40px] ${theme === "light" ? "bg-black" : "bg-white"} shadow-lg rounded-full absolute right-2 bottom-56 grid place-items-center`}
            >
              <button
                onClick={() => clickHandler(article)}
                className={`p-2 rounded-full ${theme === "light" ? "bg-black" : "bg-white"}`}
              >
                {likedArticles.some((a) => a.url === article.url) ? (
                  <FcLike fontSize="1.75rem" />
                ) : (
  ...
  return (
    <div className={`w-full min-h-screen ${theme === "light" ? "bg-black text-white" : "bg-white text-black"}`}>
      ...
      <div className="px-4 sm:px-10 lg:px-32 py-4">
        ...
        <h2 className="font-bold text-gray-500 text-lg py-2 text-center">{t("most recent news")}</h2>
