...
const Dashboard = () => {
    ...
    const fetchNews = async () => {
      ...
      try {
        ...
        if (isSearching && searchQuery) {
          url = `${API_BASE_URL}/api/v1/search?q=${encodeURIComponent(searchQuery)}&page=${currentPage}&pageSize=${PAGE_SIZE}`
        } else {
          url = `${API_BASE_URL}/api/v1/news?category=${selectedCategory}&page=${currentPage}&pageSize=${PAGE_SIZE}`
        }
  ...
  const handleBookmark = async (article) => {
    try {
      ...
      if (isBookmarked) {
        ...
        await axios.delete(`${API_BASE_URL}/api/v1/deleteBookmarkedNews/${bookmarkId}`, {
          headers: {
        ...
        const res = await axios.post(`${API_BASE_URL}/api/v1/addToBookmarkedNews`, article, {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
          },
        ...
        const savedArticle = { ...article, _id: res.data.savedArticle._id }
        updatedLikedArticles = [...likedArticles, savedArticle]
        toast.success("Added to favorites")
      }
  ...
  const renderNewsCard = (article) => {
    ...
    return (
      ...
      >
        <div
          className={`w-10 h-10 shadow-lg rounded-full absolute top-4 right-4 grid place-items-center ${
            theme === "light" ? "bg-black" : "bg-gray-800"
          }`}
        >
          <button
  ...
  return (
    <div className={`min-h-screen ${theme === "light" ? "bg-black text-white" : "bg-white text-black"}`}>
      <div className="container mx-auto px-4 py-8">
        ...
        <p className={`text-center mb-8 ${theme === "light" ? "text-gray-400" : "text-gray-600"}`}>Most recent news</p>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">{selectedNews.map(renderNewsCard)}</div>
        )}
